...
Page({
    ...
    onLoad: function(options) {
        let _this = this;
        _this.setData({
            topViewHeight: app.windowHeight - app.globalData.topbarHeight - 50,
        });
		this.commentxt='';
	...
	loadComment: function () {
		let _this = this;
		let url = loginApi.domin + '/qinggan/index.php/Home/Xiaochengxu/expert_evaluate';
		loginApi.requestUrl(_this, url, "POST", {
			page: this.page,
			len: this.rows,
	...
	uploadQuestion: function () {
		let _this = this;
		let url = loginApi.domin + '/qinggan/index.php/Home/Xiaochengxu/question';
		loginApi.requestUrl(_this, url, "POST", {
			"text": this.commentxt,
			"uid": wx.getStorageSync("u_id"),
			"expert_id": this.data.expert_id,
			...
			if (res.status == 1) {
				wx.navigateTo({
					url: '/pages/lianaiwendalist/lianaiwendalist'
				})
			}else{
				wx.showModal({
					title: '服务器异常',
